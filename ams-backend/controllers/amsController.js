   ...
   // Metric Report: Buffer Level
     sequelize.query('select  it.iteration_number as it_num,avg(mv.metric_values) as val from metric_values as mv join iterations as it on mv.iteration_id = it.id where mv.metric_id=8 and mv.test_id = ? group by mv.iteration_id ', { replacements:  [ max_test_id ] , type: sequelize.QueryTypes.SELECT }
           ...
           console.log(" Buffer Level Number :" + buffer_level_avg.it_num);
                 ...
                 res.render('index', {
          manifest_delay_detail:manifest_loading_delay,manifest_iteration_num:JSON.stringify(manifest_iteration_num), manifest_delay_graph: JSON.stringify(manifest_delay_graph), playback_delay_detail: playback_load_delay,playback_iteration_num: JSON.stringify(playback_iteration_num),playback_delay_graph:JSON.stringify(playback_delay_graph),
          dropped_frames_detail:dropped_frames,dropped_frames_iteration_num:JSON.stringify(dropped_frames_iteration_num),dropped_frames_graph:JSON.stringify(dropped_frames_graph),stream_initialization_delay_detail:stream_initialization_delay,stream_initialization_iteration_num:JSON.stringify(stream_initialization_iteration_num),stream_initialization_graph:JSON.stringify(stream_initialization_graph), quality_change_delay_details:quality_change_delay,qulity_change_iteration_num:JSON.stringify(qulity_change_iteration_num),qulity_change_graph:JSON.stringify(qulity_change_graph),playback_seeking_delay_details:playback_seeking_delay,playback_seeking_iteration_num:JSON.stringify(playback_seeking_iteration_num),playback_seeking_graph:JSON.stringify(playback_seeking_graph),buffer_stable_time_details:buffer_stable_time,buffer_stable_time_iteration_num:JSON.stringify(buffer_stable_time_iteration_num),buffer_stable_time_graph:JSON.stringify(buffer_stable_time_graph),playback_rate_details:playback_rate,playback_rate_iteration_num:JSON.stringify(playback_rate_iteration_num),playback_rate_graph:JSON.stringify(playback_rate_graph),buffer_level_avg_details:buffer_level_avg,buffer_level_avg_iteration_num:JSON.stringify(buffer_level_avg_iteration_num),buffer_level_avg_graph:JSON.stringify(buffer_level_avg_graph)});
